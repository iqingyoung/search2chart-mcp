...
function run() {
  ...
  child.on('close', () => {
    ...
    let dataHasDataUri = false;   // id=3 应含 ![](data:image/svg+xml;base64,...) 内联线
    let dataHasTypeName = false;  // id=3 summary 应含中文类型名（如"柱状图"）
    let dataHasDataBlock = false; // id=3 应含清洗数据 JSON 块
    let lineNoImage = true;       // id=6 returnImage=false：不含 image block，但仍含 A/B 内联 markdown 线
    ...
    for (const l of lines) {
      ...
      if ((m.id === 3 || m.id === 5) && m.result && m.result.content) {
        ...
        // 校验 summary 文本里含一行 ![](data:image/svg+xml;base64,...) 内联线（自包含、无需联网）
        if (m.id === 3 && /!\[[^\]]*\]\(data:image\/svg\+xml;base64,/.test(txt)) dataHasDataUri = true;
        // 校验中文类型名
        if (m.id === 3 && /类型：[柱状图|折线图|饼图]/.test(txt)) dataHasTypeName = true;
        // 校验清洗数据 JSON 块
        if (m.id === 3 && blocks.some(b => b.type === 'text' && b.text.includes('清洗后的完整数据'))) dataHasDataBlock = true;
      }
      if (m.id === 6 && m.result && m.result.content) {
        // returnImage=false：应【不含 image content block】，但【仍含 A/B 内联 markdown 图片线】(http/file)
        // —— 这正是我们要的：关掉会触发 WorkBuddy 后端报错的 image 块，同时保留对话框内联所需的 md 图片线
        const noImgBlock = !m.result.content.some(b => b.type === 'image');
        const hasInlineLine = m.result.content.some(b => b.type === 'text' && /(data:image\/svg\+xml;base64,|http:\/\/127\.0\.0\.1:)/.test(b.text));
        lineNoImage = noImgBlock && hasInlineLine;
      }
      if (m.id === 4 && m.result && m.result.content) typesOk = true;
    }

    const toolsOk = expectedTools.every(t => tools.includes(t));
    const allPass = okInit && toolsOk && !!dataFile && !!fileFile && typesOk && dataHasImage && fileHasImage && dataHasDataUri && dataHasTypeName && dataHasDataBlock && lineNoImage;
    ...
    console.log('chart_from_data dataUri:', dataHasDataUri ? 'PASS (data: SVG 内联图片)' : 'FAIL');
    ...
    console.log('returnImage=false    :', lineNoImage ? 'PASS (no image block, inline A/B kept)' : 'FAIL');
